...
- Default deployed theme is controlled by repo Variable/Secret `WALLBOARD_THEME` (`dark`/`light`/`auto`), applied by the "Apply Wallboard Theme Override" step in `sync-and-deploy.yml` which rewrites `theme:` in `site/config.js` before deploy. **Gotcha**: `secrets` context cannot be used inside a step's `if:` conditional (GitHub Actions parser error "Unrecognized named-value: 'secrets'") — only `vars`/`env` are allowed there. The empty-check must live inside the step's script/env logic instead, not in `if:`.
